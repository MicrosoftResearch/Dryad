...
#define MGDREFERENCE %
#else
#define MGDHANDLE __gc* 
#define MGDHANDLECV const volatile__gc* 
#define MGDREFERENCE __gc&
#endif
...
namespace detail
{
    ...
    {
        managed_convertible( System::Object MGDHANDLECV );
        managed_convertible( System::Enum MGDHANDLECV );
        managed_convertible( System::ValueType MGDHANDLECV );
        managed_convertible( System::Delegate MGDHANDLECV );
    };
    ...
    {
    private:
        static yes check( managed_convertible );
        static no __cdecl check( ... );
        static typename remove_ref<T>::type & make();
    public:
        static bool const value = sizeof( yes ) == sizeof( check( make() ) );
    };

    #ifdef SMART_ANY_PTS
    template<typename T>
    struct is_managed<T MGDREFERENCE>
    {
        static bool const value = true;
    };
    template<typename T>
    struct is_managed<T MGDHANDLE>
    {
        static bool const value = true;
    };
    template<typename T>
    struct is_managed<T MGDHANDLE const>
    {
        static bool const value = is_managed<T MGDHANDLE>::value;
    };
    template<typename T>
    struct is_managed<T MGDHANDLE volatile>
    {
        static bool const value = is_managed<T MGDHANDLE>::value;
    };
    template<typename T>
    struct is_managed<T MGDHANDLE const volatile>
    {
        static bool const value = is_managed<T MGDHANDLE>::value;
    };
    #endif
	...
	template<>
    struct is_managed<System::Void MGDHANDLE>
    {
        static bool const value = true;
    };
    template<>
    struct is_managed<System::Void const MGDHANDLE>
    {
        static bool const value = true;
    };
    template<>
    struct is_managed<System::Void volatile MGDHANDLE>
    {
        static bool const value = true;
    };
    template<>
    struct is_managed<System::Void const volatile MGDHANDLE>
    {
        static bool const value = true;
    };
#endif
    ...
    // Partially specialize for pointer types
    template<typename T>
    struct deref<T MGDHANDLE>
    {
        typedef T MGDREFERENCE type; // The result of dereferencing a T MGDHANDLE
    };

    // Partially specialize for pointer types
    template<typename T>
    struct deref<T MGDHANDLE const>
    {
        typedef typename deref<T MGDHANDLE>::type type; // The result of dereferencing a T MGDHANDLE
    };

    // Partially specialize for pointer types
    template<typename T>
    struct deref<T MGDHANDLE volatile>
    {
        typedef typename deref<T MGDHANDLE>::type type; // The result of dereferencing a T MGDHANDLE
    };
    ...
    // Partially specialize for pointer types
    template<typename T>
    struct deref<T MGDHANDLE const volatile>
    {
        typedef typename deref<T MGDHANDLE>::type type; // The result of dereferencing a T MGDHANDLE
    };
	...
	// Fully specialize for void*
    template<>
    struct deref<System::Void MGDHANDLE>
    {
        typedef smart_any_cannot_dereference type; // cannot dereference a System::Void MGDHANDLE
    };

    // Fully specialize for void const*
    template<>
    struct deref<System::Void const MGDHANDLE>
    {
        typedef smart_any_cannot_dereference type; // cannot dereference a System::Void MGDHANDLE
    };
    ...
    // Fully specialize for void volatile*
    template<>
    struct deref<System::Void volatile MGDHANDLE>
    {
        typedef smart_any_cannot_dereference type; // cannot dereference a System::Void MGDHANDLE
    };
    ...
    // Fully specialize for void const volatile*
    template<>
    struct deref<System::Void const volatile MGDHANDLE>
    {
        typedef smart_any_cannot_dereference type; // cannot dereference a System::Void MGDHANDLE
    };
    ...
    private:
        ...
        template<typename U> static yes check( wrap_t<U,&U::unused>* );
        template<typename U> static no  __cdecl check( ... );
    public:
    ...
    #ifdef _MANAGED
    template<typename T>
    struct is_handle_helper<T MGDREFERENCE>
    {
        static bool const value = false;
    };
    #endif
...
struct close_delete
{
    ...
    #ifdef _MANAGED
    template<typename T>
    static void close( gcroot<T MGDHANDLE> const & p )
    {
        delete static_cast<T MGDHANDLE>( p );
    }
    #endif
};
