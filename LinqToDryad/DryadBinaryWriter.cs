...
{
    /// <summary>
    /// HpcBinaryWriter is the main interface for user provided custom serializers
    /// or DL-internal autoserialization codepaths to write primitive types from a partition file.
    /// </summary>
    public unsafe sealed class HpcBinaryWriter
    {
        ...
        internal HpcBinaryWriter(IntPtr vertexInfo, UInt32 portNum, Int32 buffSize)
            : this(new HpcLinqChannel(vertexInfo, portNum, false), Encoding.UTF8, buffSize)
        {
        }

        internal HpcBinaryWriter(IntPtr vertexInfo, UInt32 portNum, Encoding encoding, Int32 buffSize)
        ...
        internal UInt64 GetFingerPrint()
        {
            if (!this.m_calcFP)
            {
                throw new DryadLinqException(HpcLinqErrorCode.FingerprintDisabled,
        ...
        public void WriteBytes(byte[] byteBuffer, int offset, int byteCount)
        {
            ...
            {
                HpcLinqUtil.memcpy(pBytes + offset, this.m_curDataBlock + this.m_curRecordEnd, byteCount);
            }
        ...
        public void WriteRawBytes(byte* pBytes, Int32 numBytes)
        {
            ...
            HpcLinqUtil.memcpy(pBytes, this.m_curDataBlock + this.m_curRecordEnd, numBytes);
            this.m_curRecordEnd += numBytes;
        }
    }
}
...
{
    // internal adapter class to make a HpcBinaryWriter work as a Stream
    // this is needed to reuse Stream-based serialization code.
    internal class HpcBinaryWriterToStreamAdapter : Stream
    {
        private HpcBinaryWriter m_dbw;

        internal HpcBinaryWriterToStreamAdapter(HpcBinaryWriter dbw)
        {
            m_dbw = dbw;
        }
        ...
        {
            get { return m_dbw.Length; }
            set { throw new DryadLinqException(HpcLinqErrorCode.SettingPositionNotSupported,
                                             SR.SettingPositionNotSupported); }
        }

        public override int Read(byte[] buffer, int offset, int count)
        {
            throw new DryadLinqException(HpcLinqErrorCode.ReadNotAllowed, SR.ReadNotAllowed);
        }

        public override long Seek(long offset, SeekOrigin origin)
        {
            throw new DryadLinqException(HpcLinqErrorCode.SeekNotSupported, SR.SeekNotSupported);
        }
