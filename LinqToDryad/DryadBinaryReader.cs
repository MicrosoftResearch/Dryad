...
{
    /// <summary>
    /// HpcBinaryReader is the main interface for user provided custom serializers
    /// or DL-internal autoserialization codepaths to read primitive types from a partition file.
    /// </summary>
    public unsafe sealed class HpcBinaryReader
    {
        ...
        private Int32 m_curBlockPos;    // Current position on the read buffer. This is updated by individual ReadXXX methods as they pull bytes for primitives they are reading
        
        private bool m_isClosed;
        
        internal HpcBinaryReader(NativeBlockStream stream)
            : this(stream, Encoding.UTF8)
        {
        }

        internal HpcBinaryReader(NativeBlockStream stream, Encoding encoding)
        {
        ...
        internal  HpcBinaryReader(IntPtr vertexInfo, UInt32 portNum)
            : this(new HpcLinqChannel(vertexInfo, portNum, true), Encoding.UTF8)
        {
        }

        internal HpcBinaryReader(IntPtr vertexInfo, UInt32 portNum, Encoding encoding)
            : this(new HpcLinqChannel(vertexInfo, portNum, true), encoding)
        {
        }
        ...
        /// <summary>
        /// Helper used by HpcRecordReader and generated vertex code to check whether the
        ...
        public byte ReadUByte()
        {
            if (this.m_curBlockPos == this.m_curBlockSize)
            {
                ...
                if (this.m_curBlockSize <= 0)
                {
                    throw new DryadLinqException(HpcLinqErrorCode.EndOfStreamEncountered,
        ...
        public unsafe int ReadChars(char[] destBuffer, int offset, int charCount)
        {
            ...
            while (numChars > 0)
            {
                ...
                Debug.Assert(numAvailBytes == 0);   // if we've reached this line there must be 0 bytes remaining, if not there's a mismatch in the math above
        ...
        public int ReadBytes(byte[] destBuffer, int offset, int byteCount)
        {
            ...
            {                
                ...
                {                    
                    ...
                    if (numAvailBytes >= numBytes)
                    {
                        HpcLinqUtil.memcpy(this.m_curDataBlock + this.m_curBlockPos,
                                           pBytes + numBytesRead,
                                           numBytes);
                        this.m_curBlockPos += numBytes;
